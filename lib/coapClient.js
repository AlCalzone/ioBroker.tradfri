...
var CoapClient = function () {
	function CoapClient(endpoint, callbackFn, userObj) {
		...
		_nodeCoapClient.CoapClient.setSecurityParams();
	}
	...
	_createClass(CoapClient, [{
		...
		value: function value() {
			...
			_nodeCoapClient.CoapClient.request(reqOpts, dtlsOpts, function (req) {
		...
		value: function value() {
			...
			// begin request
			var reqOpts = getRequestOptions(method, this.endpoint, true);
			_global2.default.log(`requesting coap endpoint ${reqOpts.hostname}${reqOpts.pathname}`);
