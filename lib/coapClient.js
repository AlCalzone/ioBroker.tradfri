...
var CoapClient = function () {
	function CoapClient(endpoint, callbackFn) {
	...
	_createClass(CoapClient, [{
		...
		value: function request() {
			...
			// begin request
			var requestOptions = getRequestOptions(method, this.endpoint, false);
			_global2.default.log(`requesting coap endpoint ${requestOptions.hostname}${requestOptions.pathname}`);

			_coapDtls2.default.request(requestOptions, getDTLSOptions(), function (req) {
		...
		value: function observe() {
			...
			// begin request
			var requestOptions = getRequestOptions(method, this.endpoint, true);
			_global2.default.log(`trying to observe coap endpoint ${requestOptions.hostname}${requestOptions.pathname}`);
			...
			_coapDtls2.default.request(requestOptions, getDTLSOptions(), function (req) {
				req.on("response", function (res) {
					...
					res.on("data", function (data) {
						...
						if (_this2[callback]) _this2[callback](data);
					});
					// also notify our creator
					if (_this2[callback]) _this2[callback](res.payload);
					// TODO: handle errors
				});
				// potentially write payload
				if (_global2.default.isdef(payload)) req.write(JSON.stringify(payload));
